...
import { createGoldVouchers, createVouchers } from '../../utils/voucherActions';
import styles from './voucher.module.css';
import { faPrint } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';

const GoldVoucherModal = ({ setModal, data, setShowToster, router }) => {
    const printRefCodes = useRef(null);
    const { allCodes, parsedTime } = data;
    const handleSubmit = async (e) => {
        e.preventDefault();
        await createGoldVouchers(data, setShowToster, router);
    }
    ...
    return (
        <form className={styles.voucherForm} onSubmit={handleSubmit}>
            <div className={styles.voucherContent}>
                ...
                <div ref={printRefCodes} className={styles.voucherBody}>
                    {allCodes.map((code) => {
                        return (
                            <>
...
export default GoldVoucherModal